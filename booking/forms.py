...
class BookTableForm(forms.Form):
    ...
    time = forms.CharField(label="Time", help_text="",
                           widget=forms.TimeInput(attrs={'class':'form-control timepicker', 'type':'text',
    ...
    def clean(self):
        ...
        time = datetime.strptime(cleaned_data.get("time"), '%H:%M')

        if datetime.combine(date, time.time()) < now:
        ...
        if time.time() < restaurant_opens.time() or time.time() > restaurant_closes.time():
